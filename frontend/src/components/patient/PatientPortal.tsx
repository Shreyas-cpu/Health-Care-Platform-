...
import type { Appointment, Doctor, PatientDocument } from '../../types'
...
export const PatientPortal: React.FC<PatientPortalProps> = ({ session, onRequireLogin }) => {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      const slotStart = `${bookingDate}T${selectedSlot}:00Z`
      const slotEnd = `${bookingDate}T${selectedSlot}:00Z` // backend engine handles interval
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {vaultDocs.map((doc) => (
                  ...
                  >
                    ...
                    <a
                      href={`http://localhost:9000/patient-documents/${doc.s3_key}`}
                      target="_blank"
                      rel="noopener noreferrer"
