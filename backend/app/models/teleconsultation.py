...
class TeleconsultationSession(Base, TimestampMixin):
    ...
    status: Mapped[SessionStatus] = mapped_column(
        ...
        default=SessionStatus.SCHEDULED, nullable=False, index=True,
    )
    patient_joined_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    doctor_joined_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    session_started_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    session_ended_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    recording_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    doctor_recording_consent: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    patient_recording_consent: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
