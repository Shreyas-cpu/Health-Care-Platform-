...
async def save_clinic(
    data: ClinicCreateOrUpdate, current_user: User = Depends(require_roles(UserRole.DOCTOR)),
...
async def toggle_listing(
    data: ToggleListingRequest, current_user: User = Depends(require_roles(UserRole.DOCTOR)),
    session: AsyncSession = Depends(get_db),
):
    try:
        return await toggle_doctor_listing(session, current_user.id, data.listing_online)
...
async def toggle_video(
    data: ToggleVideoRequest, current_user: User = Depends(require_roles(UserRole.DOCTOR)),
    session: AsyncSession = Depends(get_db),
):
    try:
