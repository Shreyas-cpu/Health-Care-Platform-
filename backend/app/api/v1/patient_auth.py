...
async def send_otp(req: SendOTPRequest):
    success, message = await identity_service.request_otp(
        req.phone_number, purpose="patient_login"
    )
    if not success:
        raise HTTPException(
...
async def verify_otp(
    ...
    user = (
        await session.execute(select(User).where(User.phone_number == req.phone_number))
    ).scalar_one_or_none()
    if not user:
        user = User(
            id=uuid.uuid4(),
        ...
        raise HTTPException(
            ...
            detail="Phone number belongs to a non-patient account.",
        )
...
async def get_me(
    ...
    patient = (
        await session.execute(select(Patient).where(Patient.user_id == current_user.id))
    ).scalar_one_or_none()
    if not patient:
        raise HTTPException(
