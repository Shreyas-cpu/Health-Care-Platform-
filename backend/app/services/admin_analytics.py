...
async def get_platform_telemetry(session: AsyncSession) -> PlatformTelemetryResponse:
    verified = (
        await session.execute(
            select(func.count(Doctor.user_id)).where(
    ...
    patient_docs = (
        await session.execute(select(func.count(PatientDocument.id)))
    ).scalar_one()
    gtv = (
        await session.execute(
    ...
    if not gtv or Decimal(str(gtv)) == Decimal(0):
        gtv = (
            await session.execute(
                select(func.coalesce(func.sum(Appointment.fee_amount), 0)).where(
                    (Appointment.status == AppointmentStatus.COMPLETED)
