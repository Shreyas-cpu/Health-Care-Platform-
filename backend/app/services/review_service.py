    ...
    count, average = (
        await session.execute(
            select(func.count(Review.id), func.avg(Review.rating)).where(
                Review.doctor_id == doctor_id, Review.status == ReviewStatus.PUBLISHED
            )
        )
    ).one()
...
async def submit_review(
    patient_id: uuid.UUID, payload: ReviewCreate, session: AsyncSession
) -> Review:
    appointment = (
        await session.execute(
    ...
    if appointment.patient_id != patient_id:
        raise HTTPException(403, "Only the appointment patient may submit a review.")
    if appointment.status != AppointmentStatus.COMPLETED:
        raise HTTPException(
            400, "Reviews are available only after consultation completion."
        )
    if (
        await session.execute(
        ...
        raise HTTPException(409, "A review already exists for this appointment.")
    review = Review(
    ...
    session.add(review)
    await session.flush()
    await recalculate_doctor_rating(appointment.doctor_id, session)
...
async def moderate_review(
    ...
    await record_audit_log(
        ...
        action=f"review_{action.lower()}",
        reason=reason_text,
        session=session,
        previous_state={"status": review.status.value},
...
async def list_reviews_for_moderation(
    status_filter: ReviewStatus | None, session: AsyncSession
) -> list[Review]:
